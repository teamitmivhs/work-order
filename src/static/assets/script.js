...
// Placeholder for Kaizen evaluation rendering
function renderKaizenEvaluation() {
  // TODO: Implement Kaizen evaluation rendering logic
  console.warn('renderKaizenEvaluation is not yet implemented.');
}
...
function stopWorkOrderTimer(orderId) {
  ...
  if (orderIndex !== -1) {
    const order = workOrders[orderIndex]
    if (order.startTime) {
      order.accumulatedDuration =
        (order.accumulatedDuration || 0) + (Date.now() - order.startTime)
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  function addHelperOperator(memberId) {
    if (!additionalOperators.includes(memberId)) {
      ...
      showPopup(
        ...
        `${members.find((m) => m.id === memberId).name} ditambahkan sebagai operator bantuan.`,
        'success'
      )
  ...
  function populateStandbyOperatorsInTakeOrderPopup() {
    ...
    uniqueSelectedOperatorIds.forEach((memberId) => {
      ...
      if (member) {
        ...
        operatorDiv.className =
          'flex items-center gap-3 p-2 bg-blue-50 rounded-lg shadow-sm'
  ...
  function removeHelperOperator(memberId) {
    ...
    showPopup(
      'Operator Dihapus',
      `${members.find((m) => m.id === memberId).name} dihapus dari operator bantuan.`,
      'info'
    )
  ...
  orderLocationSelect.addEventListener('change', function () {
    ...
    if (selectedLocation) {
      ...
      const prompt =
        locationPrompts[selectedLocation] || locationPrompts.default
  ...
  createOrderForm.addEventListener('submit', function (e) {
    ...
    // Gabungkan lokasi
    const finalLocation = specificLocation
    ...
    fetch('/api/workorders', {
      ...
      .then((data) => {
        ...
        showPopup(
          ...
          `Work Order #${newOrderId} telah berhasil dibuat dan disimpan di database.`,
          'success'
        )
      })
      .catch((error) => {
  ...
  function initializeMemberImages() {
    members.forEach((member) => {
      ...
      if (statusContainer) {
        const memberImagesContainer =
          statusContainer.querySelector('.member-images')
        const memberImg = document.createElement('img')
        const avatar = `/static/public/${member.avatar}`
  ...
  function openTakeOrderPopup(orderId) {
    ...
    if (!currentUser || !currentUser.id) {
      ...
      if (!currentUser || !currentUser.id) {
        showPopup(
          'Error',
          'Tidak dapat mengambil order, data pengguna tidak ditemukan. Silakan refresh halaman.',
          'error'
        )
        return
      }
    ...
    if (order.executors && order.executors.includes(currentUser.id)) {
      showPopup(
        'Peringatan',
        'Anda sudah terdaftar sebagai pelaksana untuk order ini!',
        'warning'
      )
      return
    }
  ...
  function populateAvailableWorkersForAddWorker(existingExecutorIds) {
    ...
    const availableMembers = members.filter(
      (m) => m.status === 'standby' && !existingExecutorIds.includes(m.id)
    )
  ...
  function addWorkerToOrder(memberId) {
    ...
    if (currentOrder.executors.length >= 4) {
      showPopup(
        'Peringatan',
        'Satu order hanya dapat dikerjakan oleh maksimal 4 orang.',
        'warning'
      )
      return
    }
    ...
    showPopup(
      'Worker Ditambahkan',
      `${members.find((m) => m.id === memberId).name} berhasil ditambahkan sebagai worker untuk order #${currentOrder.id}.`,
      'success'
    )
  }
  ...
  function populateSafetyChecklist(location) {
    ...
    const items =
      safetyChecklistItems[mainLocation] || safetyChecklistItems.default
  ...
  function confirmTakeOrder() {
    ...
    const allAssignedOperatorIds = [
      ...new Set([currentUser.id, ...additionalOperators]),
    ]
    ...
    showPopup(
      ...
      `Berhasil mengambil order #${currentOrder.id}! Anda sekarang terdaftar sebagai pelaksana order ini.`,
      'success'
    )
    ...
    fetch(`/api/workorders/${currentOrder.id}/take`, {
      ...
      .then((data) => {
        ...
        showPopup(
          'Order Berhasil Diambil!',
          `Berhasil mengambil order #${data.order_id}!`,
          'success'
        )
  ...
  function markOrderDone(orderId) {
    ...
    fetch(`/api/workorders/${orderId}/complete`, {
      ...
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            'Gagal menyelesaikan order. Status: ' + response.status
          )
        }
      ...
      function () {
        ...
        fetch(`/api/workorders/${orderId}`, {
          ...
          .then((response) => {
            if (!response.ok) {
              throw new Error(
                'Gagal menghapus order. Status: ' + response.status
              )
            }
            return response.json()
          })
  ...
  function updateSummaryCounts() {
    ...
    ).length // Explicitly count completed
  ...
  function populateSearchResults(searchTerm = '') {
    ...
    filteredMembers.forEach((member) => {
      ...
      memberItem.className =
        'flex items-center gap-3 px-4 py-2 hover:bg-gray-100 cursor-pointer transition-colors'
  ...
  function highlightMember(memberId) {
    ...
    const memberImg = document.querySelector(
      `img[data-member-id="${memberId}"]`
    )
    if (memberImg) {
      memberImg.classList.add(
        'member-highlight',
        'ring-2',
        'ring-blue-500',
        'ring-offset-2'
  ...
  function updateStatusUI(member, oldStatus, newStatus) {
    ...
    if (oldStatusContainer) {
      const memberImg = oldStatusContainer.querySelector(
        `img[data-member-id="${member.id}"]`
      )
      if (memberImg) {
        memberImg.remove()
    ...
    if (newStatusContainer) {
      const memberImagesContainer =
        newStatusContainer.querySelector('.member-images')
      const memberImg = document.createElement('img')
      const avatar = `/static/public/${member.avatar}`
      memberImg.src = avatar
      memberImg.alt = member.name
